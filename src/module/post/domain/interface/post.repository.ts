...
export interface IPostRepository {
  ...
  findAll(limit: number, page: number): Promise<PaginatedResponseRepository<Post>>;
  findPostsByType(type: MediaType, limit: number, page: number): Promise<PaginatedResponseRepository<Post>>;
  update(id: string, updateDto: UpdatePostDto): Promise<Post>;
  updateIsPublished(postId: string, isPublished: boolean): Promise<Post>;
  search(query: string, limit: number, page: number): Promise<PaginatedResponseRepository<Post>>;
